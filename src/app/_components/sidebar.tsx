...
function Sidebar({
  ...
  const { userChats, setUserChats, selectedChat, setSelectedChat } = chatsGlobalStore() as any;
  ...
  const deleteChatHandler = async (chatId: string) => {
    try {
      ...
      if (response.success) {
        ...
        message.error("Something wrong when deleting the chat!")
      }
    } catch (error: any) {
  ...
  return (
    <div className="w-80 bg-sidebar p-5 flex flex-col justify-between h-full">
      <div className='flex-1'>
          <div className="flex gap-2 border border-gray-200 border-solid text-gray-200 p-2 rounded-sm w-max text-sm items-center cursor-pointer"
            onClick={() => {
              setSelectedChat(null);
        ...
        <div className="flex flex-col mt-7">
          ...
          {!loading && userChats?.length === 0 && (
            <span className="text-gray-400 text-xs px-2 py-5">No Chats Found</span>
          )}

          <div className="flex flex-col gap-3 mt-7">
            {userChats.map((chat: any) => (
              <div className={classNames('cursor-pointer flex justify-between items-center p-2 hover:bg-gray-500 bg-opacity-30', {
                'bg-gray-600 rounded bg-opacity-60': selectedChat?._id === chat._id
              })}
              ...
              >
                ...
                {hoveredChatId === chat._id &&
                  <Trash2 size={15} className="text-gray-300" onClick={() => deleteChatHandler(chat._id)}></Trash2>}

                {selectedChatForDelete === chat.id &&
      ...
      <div className="flex flex-col gap-1 p-3">
        <span className="text-gray-400 text-xs"> Designed with <Heart size={14} /> by </span>
        <hr className='border border-gray-500 border-solid w-10'></hr>
        <span className="text-gray-400 text-xs"> GenSpark Team </span>
      </div>
    </div>
  )
}
